...
func TestSchedule(t *testing.T) {
	...
	cases := map[string]scheduleTestCase{
		...
		"InsufficientQuota condition set when workload cannot be scheduled because other workload in same CQ is using quota and ConfigurablePreemption is enabled": {
			featureGates: map[featuregate.Feature]bool{
		...
		"QuotaReclaimRequired condition set when workload cannot be scheduled because borrower from another CQ is using quota and ConfigurablePreemption is enabled": {
			featureGates: map[featuregate.Feature]bool{
				features.ConfigurablePreemption: true,
			},
			additionalClusterQueues: []kueue.ClusterQueue{
		...
		"QuotaReclaimRequired condition not set when ConfigurablePreemption is disabled": {
			additionalClusterQueues: []kueue.ClusterQueue{
				*utiltestingapi.MakeClusterQueue("reclaim-cq").
					Preemption(kueue.ClusterQueuePreemption{
						WithinClusterQueue: kueue.PreemptionPolicyLowerPriority,
