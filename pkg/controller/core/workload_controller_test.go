...
func TestReconcile(t *testing.T) {
	...
	cases := map[string]reconcileTestCase{
		...
		"admission resets QuotaReclaimRequired condition when ConfigurablePreemption is on": {
			featureGates: map[featuregate.Feature]bool{features.ConfigurablePreemption: true},
			workload: utiltestingapi.MakeWorkload("wl", "ns").
				ReserveQuotaAt(utiltestingapi.MakeAdmission("q1").Obj(), now).
				AdmissionCheck(kueue.AdmissionCheckState{
				...
				Condition(metav1.Condition{
					...
					Message:            "insufficient unused quota for cpu in flavor default, 2 more needed",
					LastTransitionTime: metav1.NewTime(now),
				}).
				Obj(),
			wantWorkload: utiltestingapi.MakeWorkload("wl", "ns").
		...
		"admission resets InsufficientQuota condition when ConfigurablePreemption is on": {
			featureGates: map[featuregate.Feature]bool{features.ConfigurablePreemption: true},
			workload: utiltestingapi.MakeWorkload("wl", "ns").
				ReserveQuotaAt(utiltestingapi.MakeAdmission("q1").Obj(), now).
				AdmissionCheck(kueue.AdmissionCheckState{
